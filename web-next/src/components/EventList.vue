<template>
  <div class="space-y-2">
    <div v-if="loading" class="flex items-center justify-center gap-3 py-8">
      <span class="spinner" />
      <span class="text-soft text-sm">Loading events...</span>
    </div>
    <div v-else-if="events.length === 0" class="text-center py-8">
      <span class="text-muted text-sm">No events found</span>
    </div>
    <table v-else class="w-full text-sm">
      ...
      <tbody>
        ...
        >
          ...
          <td class="py-2 text-soft">{{ formatDuration(event.Length) }}</td>
